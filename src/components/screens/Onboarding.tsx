...
function getSeasonalPreview(month: number): { icon: string; label: string; tasks: string[] } {
  ...
  if (month === 9) return {
    icon: '🌷',
    label: 'Сентябрь — осенние посадки',
    tasks: ['Посадка луковиц тюльпанов, нарциссов, крокусов', 'Уборка урожая и закладка на хранение', 'Внесение органики и подзимний посев зелени'],
  }
  if (month === 10) return {
    icon: '🍁',
    label: 'Октябрь — закрытие сезона',
    tasks: ['Посадка гиацинтов и поздних тюльпанов', 'Укрытие роз и выкопка георгинов', 'Подзимний посев чеснока, мульчирование'],
  }
  return {
    icon: '❄️',
